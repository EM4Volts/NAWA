...
import json
import zlib
import yamm_data.fileporters.ioUtils as ioUtils
...
class Message:
    ...
    def from_json(self, json, seq_number, symbols, fonts_dict):
...
class MCD:
    ...
    def generate_kernings(self):
                            ...
                            if kerning != 0:
                                ...
                                if next_val < 0x8000:
                                    ...
                                    if char + next_char in self.kernings[font.id]:
                                        ...
                                        self.kernings[font.id][char + next_char] = kerning
                            #self.kernings[font.id][char] = kerning
                            idx += 2  # skip kerning
                        elif val == 0x8001:
                            #result += f"[SET_FONT:{self.content[idx+1]}]"
                            idx += 2  # skip font id
        ...
        with open("kerning_test.json", "w") as f:
            json_str = json.dumps(self.kernings, indent=4)
            f.write(json_str)
    ...
    def update_from_json(self, json):
        ...
        for message in json["messages"]:
            self.messages.append(Message().from_json(message, seq_number, self.symbols, self.fonts_Dict))
            seq_number += 1
